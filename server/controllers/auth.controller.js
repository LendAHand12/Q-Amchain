...
import { sendVerificationEmail, sendResetPasswordEmail } from "../utils/email.service.js";
...
export const register = async (req, res) => {
  try {
    ...
    // Check if user exists with duplicate unique fields (exclude deleted users)
    // Note: fullName can be duplicated, so we don't check it
    const conditions = [
      { email, isDeleted: { $ne: true } },
      { username: username.trim().toLowerCase(), isDeleted: { $ne: true } },
      { phoneNumber: phoneNumber.trim(), isDeleted: { $ne: true } },
      { walletAddress: walletAddress.trim(), isDeleted: { $ne: true } },
      { identityNumber: identityNumber.trim(), isDeleted: { $ne: true } },
    ];
    ...
    if (existingUser) {
      ...
      if (existingUser.username === username.trim().toLowerCase()) duplicateField = "Username";
      else if (existingUser.email === email) duplicateField = "Email";
      else if (existingUser.phoneNumber === phoneNumber.trim()) duplicateField = "Phone number";
      else if (existingUser.identityNumber === identityNumber.trim()) duplicateField = "Identity number";
      else if (existingUser.walletAddress === walletAddress.trim()) duplicateField = "Wallet address";
